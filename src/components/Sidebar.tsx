        ...
        <div className="flex items-center">
          {isConnected ? (
            <div className="flex items-center justify-between items-center w-full space-x-6">
              ...
              <button
                className="bg-transparent border-0 p-1 cursor-pointer hover:bg-gray-700 rounded-md transition-colors ml-auto"
                aria-label="Logout"
                onClick={handleLogout}
                title="Disconnect"
              >
                <LogOut size={18} color="#888" className="hover:text-red-400 transition-colors" />
              </button>
            </div>
          ) : (
            <button className="px-3 py-2 bg-blue-500 rounded-md hover:bg-blue-600 transition text-white font-medium text-sm" onClick={() => openConnectModal && openConnectModal()}>
              Connect Wallet
            </button>
          )}
        </div>
